...
public class TransactionsEndpointTests
{
    private static readonly TimeSpan DefaultTimeout = TimeSpan.FromSeconds(60);

    [Fact]
    public async Task CanReadTransactionsForAccessibleAccount()
    {
        ...
        var (token, _) = await RegisterAndLoginAsync(httpClient, cancellationToken);
        ...
        {
            new TransactionImportRow(new DateOnly(2026, 1, 5), new DateOnly(2026, 1, 6), "Latest transaction", -50.25m, 949.75m),
            new TransactionImportRow(new DateOnly(2026, 1, 4), new DateOnly(2026, 1, 4), "Earlier transaction", 100m, 1000m)
        }, cancellationToken);

        var response = await httpClient.GetAsync($"/api/transactions/{accountId}", cancellationToken);

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);

        var transactions = await response.Content.ReadFromJsonAsync<List<TransactionListItemDto>>(cancellationToken);
        Assert.NotNull(transactions);
        Assert.Equal(2, transactions.Count);

        Assert.Equal("Latest transaction", transactions[0].Description);
        Assert.Equal(new DateOnly(2026, 1, 5), transactions[0].BookingDate);
        Assert.Equal(new DateOnly(2026, 1, 6), transactions[0].TransactionDate);
        Assert.Equal(-50.25m, transactions[0].Amount);
        Assert.Equal(949.75m, transactions[0].Balance);

        Assert.Equal("Earlier transaction", transactions[1].Description);
    }

    [Fact]
    public async Task CannotReadTransactionsForInaccessibleAccount()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        ...
        var (otherToken, _) = await RegisterAndLoginAsync(otherUserClient, cancellationToken);
        ...
        {
            new TransactionImportRow(new DateOnly(2026, 2, 1), new DateOnly(2026, 2, 1), "Hidden transaction", 42m, 42m)
        }, cancellationToken);

        var response = await httpClient.GetAsync($"/api/transactions/{inaccessibleAccountId}", cancellationToken);

        Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
    }

    [Fact]
    public async Task CannotReadTransactionsWithoutAuthentication()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        ...
        var response = await httpClient.GetAsync("/api/transactions/1", cancellationToken);

        Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
    }

    private static async Task<(string token, string userId)> RegisterAndLoginAsync(HttpClient httpClient, CancellationToken cancellationToken)
    {
        ...
        return (authResponse.Token, GetUserIdFromJwt(authResponse.Token));
    }
    ...
    {
        var cells = values
            .Select((value, index) => BuildCell($"{GetColumnName(index + 1)}{rowIndex}", value));
