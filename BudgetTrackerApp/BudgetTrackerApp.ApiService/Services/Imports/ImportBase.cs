...
{
    private readonly List<string> _rowWarnings = [];
    public IReadOnlyList<string> RowWarnings => _rowWarnings;
    ...
    public IEnumerable<TransactionImportDto> ParseExcelFile(Stream fileStream)
    {
        ...
        // Parse data rows (starting from row 3). Invalid rows are reported and skipped.
        while (reader.Read())
        {
            TransactionImportDto? transaction;
            try
            {
                transaction = ParseTransaction(reader, matchedColumns);
            }
            catch (InvalidOperationException ex)
            {
                _rowWarnings.Add($"Row {reader.Depth + 1} skipped: {ex.Message}");
                continue;
            }
